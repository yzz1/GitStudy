...
namespace Log4Qt
{
    ...
    void Logger::removeAppender(Appender* pAppender)
    {
        ...
        if (n == 0)
        {
            logger()->warn("Request to remove Appender '%2', which is not part of Logger '%1' appenders", name(),
                           p_appender->name());
            return;
        }
    }
    ...
    {
        Q_ASSERT_X(LogManager::rootLogger()->level() != Level::NULL_INT, "Logger::effectiveLevel()",
                   "Root logger level must not be NULL_INT");
        QReadLocker locker(&mObjectGuard);
