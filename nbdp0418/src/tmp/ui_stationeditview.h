/********************************************************************************
** Form generated from reading UI file 'stationeditview.ui'
**
** Created: Wed May 4 11:49:55 2016
**      by: Qt User Interface Compiler version 4.8.1
**
** WARNING! All changes made in this file will be lost when recompiling UI file!
********************************************************************************/

#ifndef UI_STATIONEDITVIEW_H
#define UI_STATIONEDITVIEW_H

#include <QtCore/QVariant>
#include <QtGui/QAction>
#include <QtGui/QApplication>
#include <QtGui/QButtonGroup>
#include <QtGui/QFrame>
#include <QtGui/QGridLayout>
#include <QtGui/QHBoxLayout>
#include <QtGui/QHeaderView>
#include <QtGui/QLabel>
#include <QtGui/QLineEdit>
#include <QtGui/QPushButton>
#include <QtGui/QSpacerItem>
#include <QtGui/QToolButton>
#include <QtGui/QWidget>
#include <widgets/contactsinput.h>
#include "widgets/titlewidget.h"

QT_BEGIN_NAMESPACE

class Ui_StationEditView
{
public:
    TitleWidget *tiltewidget;
    QWidget *gridLayoutWidget;
    QGridLayout *gridLayout;
    QLabel *countryLabel;
    QLabel *callsignLabel;
    QLabel *callcodeLabel;
    QLineEdit *callsign;
    QLineEdit *name;
    QLabel *nameLabel;
    QLineEdit *country;
    QLineEdit *callcode;
    QWidget *gridWidget_2;
    QGridLayout *frequencyGrid;
    QWidget *horizontalLayoutWidget_2;
    QHBoxLayout *horizontalLayout_4;
    QFrame *Contactsline;
    QWidget *horizontalLayoutWidget_3;
    QHBoxLayout *horizontalLayout_5;
    QSpacerItem *horizontalSpacer_8;
    QPushButton *okBtn;
    QSpacerItem *horizontalSpacer_13;
    QSpacerItem *horizontalSpacer_14;
    QPushButton *cancelBtn;
    QSpacerItem *horizontalSpacer_9;
    QPushButton *arrowUpButton;
    QPushButton *arrowDownButton;
    QWidget *controlWidget;
    QToolButton *homeTBnt;
    QToolButton *stationTBnt;
    QToolButton *individualTBnt;
    ContactsInput *widget;
    ContactsInput *widget_2;
    ContactsInput *widget_3;
    ContactsInput *widget_4;
    ContactsInput *widget_5;
    ContactsInput *widget_6;
    QLineEdit *rx2;
    QLineEdit *txHeader;
    QLineEdit *no2;
    QLineEdit *tx3;
    QLineEdit *channelHeader;
    QLineEdit *rxHeader;
    QLineEdit *channel2;
    QLineEdit *channel1;
    QLineEdit *no3;
    QLineEdit *tx2;
    QLineEdit *no;
    QLineEdit *noHeader;
    QLineEdit *channel3;
    QLineEdit *tx1;
    QLineEdit *rx1;
    QLineEdit *rx3;

    void setupUi(QWidget *StationEditView)
    {
        if (StationEditView->objectName().isEmpty())
            StationEditView->setObjectName(QString::fromUtf8("StationEditView"));
        StationEditView->resize(800, 544);
        tiltewidget = new TitleWidget(StationEditView);
        tiltewidget->setObjectName(QString::fromUtf8("tiltewidget"));
        tiltewidget->setGeometry(QRect(0, 0, 800, 45));
        gridLayoutWidget = new QWidget(StationEditView);
        gridLayoutWidget->setObjectName(QString::fromUtf8("gridLayoutWidget"));
        gridLayoutWidget->setGeometry(QRect(-10, 50, 801, 119));
        gridLayout = new QGridLayout(gridLayoutWidget);
        gridLayout->setObjectName(QString::fromUtf8("gridLayout"));
        gridLayout->setHorizontalSpacing(20);
        gridLayout->setVerticalSpacing(12);
        gridLayout->setContentsMargins(40, 5, 40, 0);
        countryLabel = new QLabel(gridLayoutWidget);
        countryLabel->setObjectName(QString::fromUtf8("countryLabel"));
        QSizePolicy sizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
        sizePolicy.setHorizontalStretch(0);
        sizePolicy.setVerticalStretch(0);
        sizePolicy.setHeightForWidth(countryLabel->sizePolicy().hasHeightForWidth());
        countryLabel->setSizePolicy(sizePolicy);
        countryLabel->setMinimumSize(QSize(144, 61));
        countryLabel->setStyleSheet(QString::fromUtf8("border-image: url(:/images/Ellipse_144_61);\n"
"font: 20pt \"Ubuntu\";\n"
"color: rgb(183, 183, 183);"));
        countryLabel->setAlignment(Qt::AlignCenter);

        gridLayout->addWidget(countryLabel, 0, 2, 1, 1);

        callsignLabel = new QLabel(gridLayoutWidget);
        callsignLabel->setObjectName(QString::fromUtf8("callsignLabel"));
        sizePolicy.setHeightForWidth(callsignLabel->sizePolicy().hasHeightForWidth());
        callsignLabel->setSizePolicy(sizePolicy);
        callsignLabel->setMinimumSize(QSize(144, 61));
        callsignLabel->setStyleSheet(QString::fromUtf8("border-image: url(:/images/Ellipse_144_61);\n"
"font: 20pt \"Ubuntu\";\n"
"color: rgb(183, 183, 183);"));
        callsignLabel->setAlignment(Qt::AlignCenter);

        gridLayout->addWidget(callsignLabel, 0, 1, 1, 1);

        callcodeLabel = new QLabel(gridLayoutWidget);
        callcodeLabel->setObjectName(QString::fromUtf8("callcodeLabel"));
        sizePolicy.setHeightForWidth(callcodeLabel->sizePolicy().hasHeightForWidth());
        callcodeLabel->setSizePolicy(sizePolicy);
        callcodeLabel->setMinimumSize(QSize(144, 61));
        callcodeLabel->setStyleSheet(QString::fromUtf8("border-image: url(:/images/Ellipse_144_61);\n"
"font: 20pt \"Ubuntu\";\n"
"color: rgb(183, 183, 183);"));
        callcodeLabel->setAlignment(Qt::AlignCenter);

        gridLayout->addWidget(callcodeLabel, 0, 3, 1, 1);

        callsign = new QLineEdit(gridLayoutWidget);
        callsign->setObjectName(QString::fromUtf8("callsign"));
        callsign->setEnabled(true);
        QSizePolicy sizePolicy1(QSizePolicy::Minimum, QSizePolicy::Fixed);
        sizePolicy1.setHorizontalStretch(0);
        sizePolicy1.setVerticalStretch(0);
        sizePolicy1.setHeightForWidth(callsign->sizePolicy().hasHeightForWidth());
        callsign->setSizePolicy(sizePolicy1);
        callsign->setMinimumSize(QSize(0, 40));
        callsign->setFocusPolicy(Qt::NoFocus);
        callsign->setStyleSheet(QString::fromUtf8("font: 15pt \"Ubuntu\";\n"
"color: rgb(190,190,190);"));
        callsign->setAlignment(Qt::AlignCenter);

        gridLayout->addWidget(callsign, 1, 1, 1, 1);

        name = new QLineEdit(gridLayoutWidget);
        name->setObjectName(QString::fromUtf8("name"));
        sizePolicy1.setHeightForWidth(name->sizePolicy().hasHeightForWidth());
        name->setSizePolicy(sizePolicy1);
        name->setMinimumSize(QSize(0, 40));
        name->setFocusPolicy(Qt::NoFocus);
        name->setStyleSheet(QString::fromUtf8("font: 15pt \"Ubuntu\";\n"
"color: rgb(190,190,190);"));
        name->setAlignment(Qt::AlignCenter);

        gridLayout->addWidget(name, 1, 0, 1, 1);

        nameLabel = new QLabel(gridLayoutWidget);
        nameLabel->setObjectName(QString::fromUtf8("nameLabel"));
        sizePolicy.setHeightForWidth(nameLabel->sizePolicy().hasHeightForWidth());
        nameLabel->setSizePolicy(sizePolicy);
        nameLabel->setMinimumSize(QSize(144, 61));
        nameLabel->setStyleSheet(QString::fromUtf8("border-image: url(:/images/Ellipse_144_61);\n"
"font: 20pt \"Ubuntu\";\n"
"color: rgb(183, 183, 183);"));
        nameLabel->setAlignment(Qt::AlignCenter);

        gridLayout->addWidget(nameLabel, 0, 0, 1, 1);

        country = new QLineEdit(gridLayoutWidget);
        country->setObjectName(QString::fromUtf8("country"));
        country->setEnabled(true);
        sizePolicy1.setHeightForWidth(country->sizePolicy().hasHeightForWidth());
        country->setSizePolicy(sizePolicy1);
        country->setMinimumSize(QSize(0, 40));
        country->setFocusPolicy(Qt::NoFocus);
        country->setStyleSheet(QString::fromUtf8("font: 15pt \"Ubuntu\";\n"
"color: rgb(190,190,190);"));
        country->setAlignment(Qt::AlignCenter);

        gridLayout->addWidget(country, 1, 2, 1, 1);

        callcode = new QLineEdit(gridLayoutWidget);
        callcode->setObjectName(QString::fromUtf8("callcode"));
        callcode->setEnabled(true);
        sizePolicy1.setHeightForWidth(callcode->sizePolicy().hasHeightForWidth());
        callcode->setSizePolicy(sizePolicy1);
        callcode->setMinimumSize(QSize(0, 40));
        callcode->setStyleSheet(QString::fromUtf8("font: 15pt \"Ubuntu\";\n"
"color: rgb(190,190,190);"));
        callcode->setAlignment(Qt::AlignCenter);

        gridLayout->addWidget(callcode, 1, 3, 1, 1);

        gridWidget_2 = new QWidget(StationEditView);
        gridWidget_2->setObjectName(QString::fromUtf8("gridWidget_2"));
        gridWidget_2->setGeometry(QRect(0, 170, 801, 216));
        frequencyGrid = new QGridLayout(gridWidget_2);
        frequencyGrid->setObjectName(QString::fromUtf8("frequencyGrid"));
        frequencyGrid->setSizeConstraint(QLayout::SetDefaultConstraint);
        frequencyGrid->setHorizontalSpacing(20);
        frequencyGrid->setVerticalSpacing(12);
        frequencyGrid->setContentsMargins(40, 10, 40, 10);
        horizontalLayoutWidget_2 = new QWidget(StationEditView);
        horizontalLayoutWidget_2->setObjectName(QString::fromUtf8("horizontalLayoutWidget_2"));
        horizontalLayoutWidget_2->setGeometry(QRect(0, 410, 801, 10));
        horizontalLayout_4 = new QHBoxLayout(horizontalLayoutWidget_2);
        horizontalLayout_4->setObjectName(QString::fromUtf8("horizontalLayout_4"));
        horizontalLayout_4->setContentsMargins(0, 0, 0, 0);
        Contactsline = new QFrame(horizontalLayoutWidget_2);
        Contactsline->setObjectName(QString::fromUtf8("Contactsline"));
        Contactsline->setStyleSheet(QString::fromUtf8("color: gray;"));
        Contactsline->setFrameShadow(QFrame::Plain);
        Contactsline->setFrameShape(QFrame::HLine);

        horizontalLayout_4->addWidget(Contactsline);

        horizontalLayoutWidget_3 = new QWidget(StationEditView);
        horizontalLayoutWidget_3->setObjectName(QString::fromUtf8("horizontalLayoutWidget_3"));
        horizontalLayoutWidget_3->setGeometry(QRect(0, 420, 801, 41));
        horizontalLayout_5 = new QHBoxLayout(horizontalLayoutWidget_3);
        horizontalLayout_5->setObjectName(QString::fromUtf8("horizontalLayout_5"));
        horizontalLayout_5->setContentsMargins(0, 0, 0, 0);
        horizontalSpacer_8 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_5->addItem(horizontalSpacer_8);

        okBtn = new QPushButton(horizontalLayoutWidget_3);
        okBtn->setObjectName(QString::fromUtf8("okBtn"));
        okBtn->setFocusPolicy(Qt::NoFocus);
        okBtn->setStyleSheet(QString::fromUtf8("QPushButton:focus{\n"
"    color:white;\n"
"}\n"
"\n"
""));
        QIcon icon;
        icon.addFile(QString::fromUtf8(":/images/images/OK.png"), QSize(), QIcon::Normal, QIcon::Off);
        okBtn->setIcon(icon);

        horizontalLayout_5->addWidget(okBtn);

        horizontalSpacer_13 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_5->addItem(horizontalSpacer_13);

        horizontalSpacer_14 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_5->addItem(horizontalSpacer_14);

        cancelBtn = new QPushButton(horizontalLayoutWidget_3);
        cancelBtn->setObjectName(QString::fromUtf8("cancelBtn"));
        cancelBtn->setFocusPolicy(Qt::NoFocus);
        cancelBtn->setStyleSheet(QString::fromUtf8("QPushButton:focus{\n"
"    color:white;\n"
"}\n"
""));
        QIcon icon1;
        icon1.addFile(QString::fromUtf8(":/images/cancel"), QSize(), QIcon::Normal, QIcon::Off);
        cancelBtn->setIcon(icon1);

        horizontalLayout_5->addWidget(cancelBtn);

        horizontalSpacer_9 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_5->addItem(horizontalSpacer_9);

        arrowUpButton = new QPushButton(StationEditView);
        arrowUpButton->setObjectName(QString::fromUtf8("arrowUpButton"));
        arrowUpButton->setGeometry(QRect(340, 390, 30, 15));
        arrowUpButton->setFocusPolicy(Qt::NoFocus);
        arrowUpButton->setAutoFillBackground(false);
        arrowUpButton->setStyleSheet(QString::fromUtf8("    border: none;\n"
"background-image: url(:/images/Arrow_up_gray);\n"
"selection-background-color: url(:/images/Arrow_up_white);"));
        arrowDownButton = new QPushButton(StationEditView);
        arrowDownButton->setObjectName(QString::fromUtf8("arrowDownButton"));
        arrowDownButton->setGeometry(QRect(420, 390, 30, 15));
        arrowDownButton->setFocusPolicy(Qt::NoFocus);
        arrowDownButton->setAutoFillBackground(false);
        arrowDownButton->setStyleSheet(QString::fromUtf8("    border: none;\n"
"background-image: url(:/images/Arrow_down_gray);\n"
"selection-background-color: url(:/images/Arrow_down_white);"));
        controlWidget = new QWidget(StationEditView);
        controlWidget->setObjectName(QString::fromUtf8("controlWidget"));
        controlWidget->setGeometry(QRect(0, 470, 800, 72));
        controlWidget->setAutoFillBackground(false);
        controlWidget->setStyleSheet(QString::fromUtf8(" background-color:rgb(10,21,31);"));
        homeTBnt = new QToolButton(controlWidget);
        homeTBnt->setObjectName(QString::fromUtf8("homeTBnt"));
        homeTBnt->setGeometry(QRect(80, 0, 141, 61));
        homeTBnt->setStyleSheet(QString::fromUtf8("QToolButton{\n"
"    border:none;\n"
"    outline: none;\n"
"    color:rgb(86, 74, 74);\n"
"\n"
"     background: transparent;\n"
"	font: 18pt \"Ubuntu\";\n"
"\n"
"}\n"
"QToolButton:focus{\n"
"    color:white;\n"
"\n"
"}\n"
"\n"
""));
        QIcon icon2;
        icon2.addFile(QString::fromUtf8(":/images/home_normal"), QSize(), QIcon::Normal, QIcon::On);
        icon2.addFile(QString::fromUtf8(":/images/home_hover"), QSize(), QIcon::Active, QIcon::On);
        icon2.addFile(QString::fromUtf8(":/images/home_normal"), QSize(), QIcon::Selected, QIcon::Off);
        icon2.addFile(QString::fromUtf8(":/images/home_hover"), QSize(), QIcon::Selected, QIcon::On);
        homeTBnt->setIcon(icon2);
        homeTBnt->setIconSize(QSize(56, 42));
        homeTBnt->setPopupMode(QToolButton::DelayedPopup);
        homeTBnt->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        stationTBnt = new QToolButton(controlWidget);
        stationTBnt->setObjectName(QString::fromUtf8("stationTBnt"));
        stationTBnt->setEnabled(false);
        stationTBnt->setGeometry(QRect(663, 0, 111, 61));
        stationTBnt->setStyleSheet(QString::fromUtf8("QToolButton{\n"
"    border:none;\n"
"    outline: none;\n"
"    color:rgb(86, 74, 74);\n"
"\n"
"     background: transparent;\n"
"	font: 18pt \"Ubuntu\";\n"
"\n"
"}\n"
"QToolButton:focus{\n"
"    color:white;\n"
"\n"
"}\n"
"\n"
""));
        QIcon icon3;
        icon3.addFile(QString::fromUtf8(":/images/images/station_white.png"), QSize(), QIcon::Normal, QIcon::On);
        icon3.addFile(QString::fromUtf8(":/images/images/station_white.png"), QSize(), QIcon::Disabled, QIcon::On);
        icon3.addFile(QString::fromUtf8(":/images/images/station_white.png"), QSize(), QIcon::Selected, QIcon::Off);
        icon3.addFile(QString::fromUtf8(":/images/images/station_white.png"), QSize(), QIcon::Selected, QIcon::On);
        stationTBnt->setIcon(icon3);
        stationTBnt->setIconSize(QSize(18, 31));
        stationTBnt->setPopupMode(QToolButton::DelayedPopup);
        stationTBnt->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        individualTBnt = new QToolButton(controlWidget);
        individualTBnt->setObjectName(QString::fromUtf8("individualTBnt"));
        individualTBnt->setGeometry(QRect(450, 0, 151, 61));
        individualTBnt->setStyleSheet(QString::fromUtf8("QToolButton{\n"
"    border:none;\n"
"    outline: none;\n"
"    color:rgb(86, 74, 74);\n"
"\n"
"     background: transparent;\n"
"	font: 18pt \"Ubuntu\";\n"
"\n"
"}\n"
"QToolButton:focus{\n"
"    color:white;\n"
"\n"
"}\n"
"\n"
""));
        QIcon icon4;
        icon4.addFile(QString::fromUtf8(":/images/images/individual_gray.png"), QSize(), QIcon::Normal, QIcon::On);
        icon4.addFile(QString::fromUtf8(":/images/images/individual_gray.png"), QSize(), QIcon::Selected, QIcon::Off);
        icon4.addFile(QString::fromUtf8(":/images/images/individual_white.png"), QSize(), QIcon::Selected, QIcon::On);
        individualTBnt->setIcon(icon4);
        individualTBnt->setIconSize(QSize(25, 31));
        individualTBnt->setPopupMode(QToolButton::DelayedPopup);
        individualTBnt->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        widget = new ContactsInput(StationEditView);
        widget->setObjectName(QString::fromUtf8("widget"));
        widget->setGeometry(QRect(404, 236, 165, 40));
        widget_2 = new ContactsInput(StationEditView);
        widget_2->setObjectName(QString::fromUtf8("widget_2"));
        widget_2->setGeometry(QRect(590, 236, 165, 40));
        widget_3 = new ContactsInput(StationEditView);
        widget_3->setObjectName(QString::fromUtf8("widget_3"));
        widget_3->setGeometry(QRect(404, 288, 165, 40));
        widget_4 = new ContactsInput(StationEditView);
        widget_4->setObjectName(QString::fromUtf8("widget_4"));
        widget_4->setGeometry(QRect(590, 288, 165, 40));
        widget_5 = new ContactsInput(StationEditView);
        widget_5->setObjectName(QString::fromUtf8("widget_5"));
        widget_5->setGeometry(QRect(404, 340, 165, 40));
        widget_6 = new ContactsInput(StationEditView);
        widget_6->setObjectName(QString::fromUtf8("widget_6"));
        widget_6->setGeometry(QRect(590, 340, 165, 40));
        rx2 = new QLineEdit(StationEditView);
        rx2->setObjectName(QString::fromUtf8("rx2"));
        rx2->setEnabled(false);
        rx2->setGeometry(QRect(590, 288, 166, 40));
        QSizePolicy sizePolicy2(QSizePolicy::Maximum, QSizePolicy::Fixed);
        sizePolicy2.setHorizontalStretch(0);
        sizePolicy2.setVerticalStretch(0);
        sizePolicy2.setHeightForWidth(rx2->sizePolicy().hasHeightForWidth());
        rx2->setSizePolicy(sizePolicy2);
        rx2->setMinimumSize(QSize(80, 40));
        QFont font;
        font.setPointSize(15);
        rx2->setFont(font);
        rx2->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        rx2->setAlignment(Qt::AlignCenter);
        txHeader = new QLineEdit(StationEditView);
        txHeader->setObjectName(QString::fromUtf8("txHeader"));
        txHeader->setEnabled(false);
        txHeader->setGeometry(QRect(404, 184, 166, 40));
        QSizePolicy sizePolicy3(QSizePolicy::Expanding, QSizePolicy::Fixed);
        sizePolicy3.setHorizontalStretch(0);
        sizePolicy3.setVerticalStretch(0);
        sizePolicy3.setHeightForWidth(txHeader->sizePolicy().hasHeightForWidth());
        txHeader->setSizePolicy(sizePolicy3);
        txHeader->setMinimumSize(QSize(80, 40));
        txHeader->setStyleSheet(QString::fromUtf8("font: 15pt \"Ubuntu\";\n"
"color: rgb(190,190,190);\n"
"background-color: rgb(19, 57, 75);"));
        txHeader->setAlignment(Qt::AlignCenter);
        txHeader->setReadOnly(true);
        no2 = new QLineEdit(StationEditView);
        no2->setObjectName(QString::fromUtf8("no2"));
        no2->setGeometry(QRect(34, 288, 165, 40));
        sizePolicy3.setHeightForWidth(no2->sizePolicy().hasHeightForWidth());
        no2->setSizePolicy(sizePolicy3);
        no2->setMinimumSize(QSize(20, 40));
        no2->setFont(font);
        no2->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        no2->setAlignment(Qt::AlignCenter);
        tx3 = new QLineEdit(StationEditView);
        tx3->setObjectName(QString::fromUtf8("tx3"));
        tx3->setEnabled(false);
        tx3->setGeometry(QRect(404, 340, 166, 40));
        sizePolicy3.setHeightForWidth(tx3->sizePolicy().hasHeightForWidth());
        tx3->setSizePolicy(sizePolicy3);
        tx3->setMinimumSize(QSize(80, 40));
        tx3->setFont(font);
        tx3->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        tx3->setAlignment(Qt::AlignCenter);
        channelHeader = new QLineEdit(StationEditView);
        channelHeader->setObjectName(QString::fromUtf8("channelHeader"));
        channelHeader->setEnabled(false);
        channelHeader->setGeometry(QRect(219, 184, 165, 40));
        sizePolicy3.setHeightForWidth(channelHeader->sizePolicy().hasHeightForWidth());
        channelHeader->setSizePolicy(sizePolicy3);
        channelHeader->setMinimumSize(QSize(80, 40));
        channelHeader->setStyleSheet(QString::fromUtf8("font: 15pt \"Ubuntu\";\n"
"color: rgb(190,190,190);\n"
"background-color: rgb(19, 57, 75);"));
        channelHeader->setAlignment(Qt::AlignCenter);
        channelHeader->setReadOnly(true);
        rxHeader = new QLineEdit(StationEditView);
        rxHeader->setObjectName(QString::fromUtf8("rxHeader"));
        rxHeader->setEnabled(false);
        rxHeader->setGeometry(QRect(590, 184, 165, 40));
        sizePolicy3.setHeightForWidth(rxHeader->sizePolicy().hasHeightForWidth());
        rxHeader->setSizePolicy(sizePolicy3);
        rxHeader->setMinimumSize(QSize(80, 40));
        rxHeader->setStyleSheet(QString::fromUtf8("font: 15pt \"Ubuntu\";\n"
"color: rgb(190,190,190);\n"
"background-color: rgb(19, 57, 75);"));
        rxHeader->setAlignment(Qt::AlignCenter);
        rxHeader->setReadOnly(true);
        channel2 = new QLineEdit(StationEditView);
        channel2->setObjectName(QString::fromUtf8("channel2"));
        channel2->setGeometry(QRect(219, 288, 165, 40));
        sizePolicy3.setHeightForWidth(channel2->sizePolicy().hasHeightForWidth());
        channel2->setSizePolicy(sizePolicy3);
        channel2->setMinimumSize(QSize(80, 40));
        channel2->setFont(font);
        channel2->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        channel2->setAlignment(Qt::AlignCenter);
        channel1 = new QLineEdit(StationEditView);
        channel1->setObjectName(QString::fromUtf8("channel1"));
        channel1->setGeometry(QRect(219, 236, 165, 40));
        sizePolicy3.setHeightForWidth(channel1->sizePolicy().hasHeightForWidth());
        channel1->setSizePolicy(sizePolicy3);
        channel1->setMinimumSize(QSize(80, 40));
        channel1->setFont(font);
        channel1->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        channel1->setAlignment(Qt::AlignCenter);
        no3 = new QLineEdit(StationEditView);
        no3->setObjectName(QString::fromUtf8("no3"));
        no3->setGeometry(QRect(34, 340, 165, 40));
        sizePolicy3.setHeightForWidth(no3->sizePolicy().hasHeightForWidth());
        no3->setSizePolicy(sizePolicy3);
        no3->setMinimumSize(QSize(20, 40));
        no3->setFont(font);
        no3->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        no3->setAlignment(Qt::AlignCenter);
        tx2 = new QLineEdit(StationEditView);
        tx2->setObjectName(QString::fromUtf8("tx2"));
        tx2->setEnabled(false);
        tx2->setGeometry(QRect(404, 288, 166, 40));
        sizePolicy3.setHeightForWidth(tx2->sizePolicy().hasHeightForWidth());
        tx2->setSizePolicy(sizePolicy3);
        tx2->setMinimumSize(QSize(80, 40));
        tx2->setFont(font);
        tx2->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        tx2->setAlignment(Qt::AlignCenter);
        no = new QLineEdit(StationEditView);
        no->setObjectName(QString::fromUtf8("no"));
        no->setGeometry(QRect(34, 236, 165, 40));
        sizePolicy3.setHeightForWidth(no->sizePolicy().hasHeightForWidth());
        no->setSizePolicy(sizePolicy3);
        no->setMinimumSize(QSize(20, 40));
        no->setFont(font);
        no->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        no->setAlignment(Qt::AlignCenter);
        noHeader = new QLineEdit(StationEditView);
        noHeader->setObjectName(QString::fromUtf8("noHeader"));
        noHeader->setEnabled(false);
        noHeader->setGeometry(QRect(34, 184, 165, 40));
        sizePolicy3.setHeightForWidth(noHeader->sizePolicy().hasHeightForWidth());
        noHeader->setSizePolicy(sizePolicy3);
        noHeader->setMinimumSize(QSize(20, 40));
        noHeader->setStyleSheet(QString::fromUtf8("font: 15pt \"Ubuntu\";\n"
"color: rgb(190,190,190);\n"
"background-color: rgb(19, 57, 75);"));
        noHeader->setAlignment(Qt::AlignCenter);
        noHeader->setReadOnly(true);
        channel3 = new QLineEdit(StationEditView);
        channel3->setObjectName(QString::fromUtf8("channel3"));
        channel3->setGeometry(QRect(219, 340, 165, 40));
        sizePolicy3.setHeightForWidth(channel3->sizePolicy().hasHeightForWidth());
        channel3->setSizePolicy(sizePolicy3);
        channel3->setMinimumSize(QSize(80, 40));
        channel3->setFont(font);
        channel3->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        channel3->setAlignment(Qt::AlignCenter);
        tx1 = new QLineEdit(StationEditView);
        tx1->setObjectName(QString::fromUtf8("tx1"));
        tx1->setEnabled(false);
        tx1->setGeometry(QRect(404, 236, 166, 40));
        sizePolicy3.setHeightForWidth(tx1->sizePolicy().hasHeightForWidth());
        tx1->setSizePolicy(sizePolicy3);
        tx1->setMinimumSize(QSize(80, 40));
        tx1->setFont(font);
        tx1->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        tx1->setAlignment(Qt::AlignCenter);
        rx1 = new QLineEdit(StationEditView);
        rx1->setObjectName(QString::fromUtf8("rx1"));
        rx1->setEnabled(false);
        rx1->setGeometry(QRect(590, 236, 166, 40));
        sizePolicy2.setHeightForWidth(rx1->sizePolicy().hasHeightForWidth());
        rx1->setSizePolicy(sizePolicy2);
        rx1->setMinimumSize(QSize(80, 40));
        rx1->setFont(font);
        rx1->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        rx1->setAlignment(Qt::AlignCenter);
        rx3 = new QLineEdit(StationEditView);
        rx3->setObjectName(QString::fromUtf8("rx3"));
        rx3->setEnabled(false);
        rx3->setGeometry(QRect(590, 340, 166, 40));
        sizePolicy2.setHeightForWidth(rx3->sizePolicy().hasHeightForWidth());
        rx3->setSizePolicy(sizePolicy2);
        rx3->setMinimumSize(QSize(80, 40));
        rx3->setFont(font);
        rx3->setStyleSheet(QString::fromUtf8("color:rgb(190,190,190)"));
        rx3->setAlignment(Qt::AlignCenter);

        retranslateUi(StationEditView);

        QMetaObject::connectSlotsByName(StationEditView);
    } // setupUi

    void retranslateUi(QWidget *StationEditView)
    {
        StationEditView->setWindowTitle(QApplication::translate("StationEditView", "Form", 0, QApplication::UnicodeUTF8));
        countryLabel->setText(QApplication::translate("StationEditView", "Country", 0, QApplication::UnicodeUTF8));
        callsignLabel->setText(QApplication::translate("StationEditView", "Callsign", 0, QApplication::UnicodeUTF8));
        callcodeLabel->setText(QApplication::translate("StationEditView", "Callcode", 0, QApplication::UnicodeUTF8));
        nameLabel->setText(QApplication::translate("StationEditView", "Name", 0, QApplication::UnicodeUTF8));
        okBtn->setText(QApplication::translate("StationEditView", "&OK", 0, QApplication::UnicodeUTF8));
        cancelBtn->setText(QApplication::translate("StationEditView", "Ca&ncel", 0, QApplication::UnicodeUTF8));
        arrowUpButton->setText(QString());
        arrowDownButton->setText(QString());
        homeTBnt->setText(QApplication::translate("StationEditView", "&Home", 0, QApplication::UnicodeUTF8));
        stationTBnt->setText(QApplication::translate("StationEditView", "Station", 0, QApplication::UnicodeUTF8));
        individualTBnt->setText(QApplication::translate("StationEditView", "&Individual", 0, QApplication::UnicodeUTF8));
        txHeader->setText(QApplication::translate("StationEditView", "Tx(KHz)", 0, QApplication::UnicodeUTF8));
        channelHeader->setText(QApplication::translate("StationEditView", "Channel", 0, QApplication::UnicodeUTF8));
        rxHeader->setText(QApplication::translate("StationEditView", "Rx(KHz)", 0, QApplication::UnicodeUTF8));
        noHeader->setText(QApplication::translate("StationEditView", "NO.", 0, QApplication::UnicodeUTF8));
    } // retranslateUi

};

namespace Ui {
    class StationEditView: public Ui_StationEditView {};
} // namespace Ui

QT_END_NAMESPACE

#endif // UI_STATIONEDITVIEW_H
