#include "managerphrasebook.h"
#include "ui_managerphrasebook.h"
...
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::ManagerPhrasebook
** 功能概述: 初始化
** 参数说明
** 作成日期：20151113
** 作成者：  HSCN
...
**
****************************************************************************/
ManagerPhrasebook::ManagerPhrasebook(QWidget* parent) :
    QWidget(parent),
    ui(new Ui::ManagerPhrasebook)
{
    Logging::get()->info("ManagerPhrasebook::ManagerPhrasebook");
    ui->setupUi(this);
    ui->contactsTiltewidget->setText(Constants::MANAGER_PHRASEBOOK_TITLE);
    m_model = new ManagerModel(this, SQLiteHelper::openConnection());
    m_model->setTable(tr(Constants::PHRASEBOOK_LIST_TABLE_NAME));
    m_model->setSort(Constants::PHRASEBOOK_INDEX_NO, Qt::AscendingOrder);
    m_model->select();
    ...
    ui->tableView->setColumnHidden(Constants::PHRASEBOOK_INDEX_UUID, true);
    ui->tableView->setColumnHidden(Constants::PHRASEBOOK_INDEX_CDATETIME, true);
    ui->tableView->setColumnHidden(Constants::PHRASEBOOK_INDEX_UDATETIME, true);
    ui->tableView->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ui->tableView->verticalHeader()->hide();
    QHeaderView* header = ui->tableView->horizontalHeader();
    header->resizeSection(Constants::PHRASEBOOK_INDEX_SHOWPIX, 100);
    header->resizeSection(Constants::PHRASEBOOK_INDEX_NO, 200);
    header->resizeSection(Constants::PHRASEBOOK_INDEX_CONTENT, 500);
...
**
****************************************************************************/
ManagerPhrasebook::~ManagerPhrasebook()
{
    Logging::get()->info("ManagerPhrasebook::~ManagerPhrasebook");
    delete ui;
}
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::eventFilter
...
**
****************************************************************************/
bool ManagerPhrasebook::eventFilter(QObject* watched, QEvent* event)
{
    Logging::get()->info("ManagerPhrasebook::eventFilter");
    QStringList nameList;
    nameList << "tableView" << "newBnt" << "editBnt" << "deleteBnt" << "optionBnt" << "homeTBnt" << "fileTBnt" <<
             "macroTBnt";
    ...
    case Qt::Key_Tab:
    {
        ...
        if (!m_optionMenuStatus && (a == Qt::Key_Down || a == Qt::Key_Tab))//移动OPTION菜单内的光标
        {
        ...
        else if (!m_optionMenuStatus && (a == Qt::Key_Right))//关闭OPTIOM菜单
        {
            QWidget* widget = findChild<QWidget*>("optionBnt");
            Button* button = static_cast<Button*>(widget);
            button->click();
...
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::showOptionMenu
** 功能概述: option菜单处理
** 参数说明
** 作成日期：20151113
** 作成者：  HSCN
...
**
****************************************************************************/
void ManagerPhrasebook::showOptionMenu()
{
    Logging::get()->info("ManagerPhrasebook::showOptionMenu");
...
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::exportClickedSlot
** 功能概述: 导出处理
** 参数说明
** 作成日期：20151113
** 作成者：  HSCN
...
**
****************************************************************************/
void ManagerPhrasebook:: exportClickedSlot()
{
    Logging::get()->info("ManagerPhrasebook::exportClickedSlot");
    ui->optionBnt->click();
    QFileDialog w;
    ...
    if (w.exec() == QFileDialog::Accepted) // ok
    {
        ...
        if (!file.isEmpty())
        {
            ...
            bool ret = SQLiteHelper::exportData("phrasebook", file);
...
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::importClickedSlot
** 功能概述: 导入处理
** 参数说明
** 作成日期：20151113
** 作成者：  HSCN
...
**
****************************************************************************/
void ManagerPhrasebook:: importClickedSlot()
{
    Logging::get()->info("ManagerPhrasebook::importClickedSlot");
    ui->optionBnt->click();
    QFileDialog w;
    w.setDirectory(QString("/"));
    w.setFilter(QString("xml Files(*.xml)"));
    ...
    for (int n = 1; n <= m_model->rowCount(); n++)
    {
        ...
        record.setValue(Constants::PHRASEBOOK_INDEX_NO, n);
...
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::deleteSelectedRow
** 功能概述: 删除记录处理
** 参数说明
** 作成日期：20151113
** 作成者：  HSCN
...
**
****************************************************************************/
void ManagerPhrasebook:: deleteSelectedRow()
{
    Logging::get()->info("ManagerPhrasebook::deleteSelectedRow");
    MessageBox  msg;
    msg.move((SCREEN_SIZE_WIDTH - msg.width()) / 2, (SCREEN_SIZE_HEIGHT - msg.height()) / 2);
...
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::newClickedSolt
** 功能概述: 新建记录
** 参数说明
** 作成日期：20151113
** 作成者：  HSCN
...
**
****************************************************************************/
void ManagerPhrasebook:: newClickedSolt()
{
    Logging::get()->info("ManagerPhrasebook::newClickedSolt");
    int ret = 0;
    QString phrasebookContent ;
    InputDialog  inputDialog;
    inputDialog.move((SCREEN_SIZE_WIDTH - inputDialog.width()) / 2, (SCREEN_SIZE_HEIGHT - inputDialog.height()) / 2);
    inputDialog.SetTitle(tr("phrase"));
    inputDialog.SetLineEditMaxLen(Constants::PHRASEBOOK_MAX_EDIT_SIZE);
    ...
    if (ret)//确认
    {
        ...
        phrasebookContent = inputDialog.getTextContent();
        int intInserRow = 0;
        ...
        m_model->setData(m_model->index(intInserRow, Constants::PHRASEBOOK_INDEX_UUID), strUUID);
        m_model->setData(m_model->index(intInserRow, Constants::PHRASEBOOK_INDEX_NO), intNo);
        m_model->setData(m_model->index(intInserRow, Constants::PHRASEBOOK_INDEX_CONTENT), phrasebookContent);
        m_model->setData(m_model->index(intInserRow, Constants::PHRASEBOOK_INDEX_CDATETIME), strDateTime);
        m_model->setData(m_model->index(intInserRow, Constants::PHRASEBOOK_INDEX_UDATETIME), strDateTime);
        m_model->submitAll();
        m_model->select();
        ui->tableView->selectRow(0);
        ui->tableView->setFocus();
...
**
****************************************************************************/
void ManagerPhrasebook:: editClickedSolt()
{
    Logging::get()->info("ManagerPhrasebook::editClickedSolt");
    ...
    if (ret)//确认
    {
        ...
        if (m_model->select())
        {
            if (m_model->rowCount() == 1)
            {
                ...
                record.setValue(Constants::PHRASEBOOK_LIST_COL_CONTENT, phrasebookContent);
                record.setValue(Constants::PHRASEBOOK_LIST__COL_UDATETIME, strDateTime);
...
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::homeClickedSlot
** 功能概述: 返回主菜单
** 参数说明
** 作成日期：20151113
** 作成者：  HSCN
...
**
****************************************************************************/
void ManagerPhrasebook:: homeClickedSlot()
{
    Logging::get()->info("ManagerPhrasebook::homeClickedSlot");
    QMap <QString, QString>paraMap;
    emit afterPageSignal(Constants::MANAGER_PHRASEBOOK, HOME_VIEW_NAME, paraMap);
}
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::macroClickedSlot
** 功能概述: 打开macro界面
** 参数说明
** 作成日期：20151113
** 作成者：  HSCN
...
**
****************************************************************************/
void ManagerPhrasebook::macroClickedSlot()
{
    Logging::get()->info("ManagerPhrasebook::macroClickedSlot");
    QMap <QString, QString>paraMap;
    emit afterPageSignal(Constants::MANAGER_PHRASEBOOK, Constants::MANAGER_MACRO, paraMap);
}
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::fileClickedSlot
** 功能概述: 打开文件列表界面
** 参数说明
** 作成日期：20151113
** 作成者：  HSCN
...
**
****************************************************************************/
void ManagerPhrasebook::fileClickedSlot()
{
    Logging::get()->info("ManagerPhrasebook::fileClickedSlot");
    QMap <QString, QString>paraMap;
    emit afterPageSignal(Constants::MANAGER_PHRASEBOOK, Constants::MANAGER_FILE, paraMap);
}
/****************************************************************************
**
** 函数名称：ManagerPhrasebook::frontPageSlot
** 功能概述: 打开页面时更新页面数据
...
**
****************************************************************************/
void ManagerPhrasebook::frontPageSlot(QString thisID, QString fowordID, QMap<QString, QString>paraMap)
{
    if (fowordID != Constants::MANAGER_PHRASEBOOK) {return;}
    
    Logging::get()->info("ManagerPhrasebook::frontPageSlot");
    m_model->select();
    m_optionMenuStatus = false;
    showOptionMenu();
